...
export function rateLimitMiddleware(
  ...
  defaultLimits: { rpm: number; tpm: number } | null
) {
  return (req: Request & { apiKey?: { id: number; rateLimitRpm: number; rateLimitTpm: number } }, res: Response, next: NextFunction) => {
    ...
    // Use key-specific limits, then defaults, then unlimited
    const rpm = apiKey.rateLimitRpm || defaultLimits?.rpm || null;
    const tpm = apiKey.rateLimitTpm || defaultLimits?.tpm || null;

    if (rpm === null && tpm === null) {
      return next(); // No limits configured — allow all requests
    }
