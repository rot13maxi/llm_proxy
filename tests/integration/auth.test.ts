...
describe('API Key Authentication', () => {
  ...
  beforeEach(async () => {
    ...
    const keyHash = await hash(validApiKey, { memoryCost: 65536, timeCost: 2, parallelism: 1 });
    db.prepare('INSERT INTO api_keys (key_prefix, key_hash, name) VALUES (?, ?, ?)').run(keyPrefix, keyHash, 'test-key');
