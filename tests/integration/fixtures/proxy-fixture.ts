...
`;
  ...
  private async createTestApiKey(): Promise<void> {
    ...
    // Generate a real API key (stored in plaintext - no hashing needed)
    const uuid = uuidv4();
    const fullKey = `sk-${uuid}`;
    const keyPrefix = uuid.slice(0, 8);
    
    db.prepare(`
      INSERT OR REPLACE INTO api_keys (key_prefix, key_value, name, is_active, rate_limit_rpm, rate_limit_tpm)
      VALUES (?, ?, 'test-key', 1, ?, ?)
    `).run(keyPrefix, fullKey, this.keyRateLimitRpm, this.keyRateLimitTpm);
