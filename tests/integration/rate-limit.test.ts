...
describe('Rate Limiting', () => {
  ...
  it('should use per-key rate limits when configured', async () => {
    ...
    // Insert key with rate limit of 3 RPM (stored in plaintext)
    db.prepare(`
      INSERT INTO api_keys (key_prefix, key_value, name, is_active, rate_limit_rpm, rate_limit_tpm)
      VALUES (?, ?, 'limited-key', 1, 3, 10000)
    `).run(keyPrefix, fullKey);
